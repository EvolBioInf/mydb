#+begin_export latex
!The package \ty{mydb} is wraps the \ty{Open} function of the
!standard library package \ty{database/sql} to allow uniform access to
...
#+begin_src go <<Open Sqlite>>=
  func OpenSqlite(name) *sql.DB, err {
	  d, e := sql.Open("sqlite3", name)
	  return d, e
  }
#+end_src
#+begin_export latex
We import \ty{sql} and a Sqlite database driver. The database driver
...
!password, host name, port, and database name.
...
#+begin_src go <<Open Mysql>>=
  func OpenMysql(user, pass, host, port, db string) *sql.DB, err {
...
!its user, password, host name, port, and database name.
...
#+begin_src go <<Open Postgresql>>=
  func OpenPostgresql(user, pass, host, port, db string) *sql.DB, err {

...
\ty{Config} contained in the \ty{mysql} package. We fill it with the
parameters we have been passed and the address we just
constructed. Then we convert that into the database source name, DSN.
#+end_export
...
#+begin_src go <<Open Postgresql>>=
  func OpenPostgresql(user, pass, host, port,
	  ...
	  templ := "postgres://%s:%s@%s:%s/%s?sslmode=disable"
	  if host == "" {
		  host = "localhost"
	  }
	  if port == "" {
...
#+end_src
#+begin_export latex
We import \ty{fmt} and the Postgresql database driver.
#+end_export
#+begin_src go <<Imports>>=
  "fmt"
...
#+begin_src go <<Testing>>=
  ...
  for i, connection := range connections {
	  fmt.Printf("Testing connection %d\n", i+1)
	  //<<Query connection>>
  }
#+end_src
#+begin_export latex
...
one attribute from the result, the genome ID. This is not a realistic
query, but it will do as our test.
#+end_export
